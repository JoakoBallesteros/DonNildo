...
export default function UsuarioForm({
  ...
  const [activo, setActivo] = useState(
    (initial.estado ?? "ACTIVO") === "ACTIVO"
  );
  const [errors, setErrors] = useState({});

  useEffect(() => {
    if (!idRol && roles.length) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            className={`w-full rounded-md border px-3 py-2 ${
              errors.dni ? "border-red-300" : "border-[#d8e4df]"
            }`}
            value={dni}
            onChange={(e) => {
        ...
        <div>
          ...
          <input
            className={`w-full rounded-md border px-3 py-2 ${
              errors.nombre ? "border-red-300" : "border-[#d8e4df]"
            }`}
            value={nombre}
            onChange={(e) => {
        ...
        <div>
          ...
          <input
            type="email"
            className={`w-full rounded-md border px-3 py-2 ${
              errors.mail ? "border-red-300" : "border-[#d8e4df]"
            }`}
            value={mail}
            onChange={(e) => {
              setMail(e.target.value);
              if (errors.mail) setErrors((x) => ({ ...x, mail: undefined }));
            }}
          />
          {errors.mail && (
        ...
        <div>
          ...
          <select
            className={`w-full rounded-md border px-3 py-2 ${
              errors.id_rol ? "border-red-300" : "border-[#d8e4df]"
            }`}
            value={idRol}
          ...
          {errors.id_rol && (
            <p className="text-xs text-red-600 mt-1">{errors.id_rol}</p>
          )}
      ...
      <div className="flex justify-end gap-2">
        <button
          onClick={onCancel}
          className="px-4 py-2 rounded-md border border-[#d8e4df] text-[#154734] bg-white"
        >
          Cancelar
        </button>
        <button
